...
# HashMap internal working
    ...
    Working of put method:
        ...
        2. Determine Bucket Index
            int index = (n - 1) & hash; // n is length of the table, always power of 2 (array)
...
# Why the order of elements in a HashMap in Java is not consistent.
    Internally, a HashMap stores entries in an array called table[].

    When you insert a key:
        hash = hash(key)
        index = (n - 1) & hash   // n is table length
        table[index] → bucket

    So order depends entirely on:
        the key’s hashCode(), its position in table[], collisions, table size, resize events.
    Change any of these → order changes.
...
# What is LinkedHashMap?
    ...
    Internal Data Structures
        ...
        LinkedHashMap entries live in TWO structures at the same time: A HashMap bucket and a doubly linked list
            So a single Entry object has:
                next → for hash bucket chaining
                before → previous entry in linked list
                after → next entry in linked list
    ...
    Internal Working: Key Operations
        ...
        5. LRU Cache Hook (Very Important)
        ...
        How It Works Internally (Java 8+)
            In Java 8 and beyond, ConcurrentHashMap uses:
                ...
                2. Fine-Grained Synchronization:
                    Instead of locking the whole map like HashTable:
...
# Iternal working of ConcurrentHashMap
    ConcurrentHashMap (CHM) allows multiple threads to read and write simultaneously
    without locking the entire map and without fail-fast iterators.

    It achieves this using:
        CAS (Compare-And-Swap) | volatile memory semantics | fine-grained locking | weakly consistent iterators
    ...
    Node Structure (Simplified)
        ...
        Notice:
            volatile fields → visibility guarantees
            final keys → immutability
    
    GET Operation (Lock-Free) map.get(key);
        Compute hash -> Read bucket index -> Traverse bucket (linked list / tree) -> Return value
        Uses volatile and no locking which makes reads extremely fast.

    PUT Operation (Fine-Grained Locking) map.put(key, value);
        If bucket empty: CAS insert
        If bucket not empty: Lock only that bucket -> Update or append node -> Unlock bucket
        No global lock and threads updating different buckets proceed concurrently.
    
        CAS vs Lock
            CAS used for:
                Empty bucket insert | Resizing coordination
            synchronized used:
                Only when collision occurs in same bucket
    
    How CHM Avoids Fail-Fast
        No modCount | No global modification counter
        Iterators don’t track structural changes, so map.put() while iterating may or may not reflect during interation.

    What Is Collections.synchronizedMap?
        Map<K,V> map = Collections.synchronizedMap(new HashMap<>());
        It is a thread-safe wrapper around an existing Map.
        How it works:
            Wraps the map in a synchronized proxy
            Every public method is synchronized on one lock
            ...
            Single global lock unline fine-grained bucket level lock like ConcurrentHashMap.
            All operations are mutually exclusive
        Fail-Fast Iterators Problem still exist because Iterator is from underlying HashMap and modCount is still present.

# How can we make an existing HashMap thread-safe without using ConcurrentHashMap? 
	Collections.synchronizedMap(new HashMap<>());
        Iteration needs to be manually synchronized
            synchronized (map) {
